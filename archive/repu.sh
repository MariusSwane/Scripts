...
function rmtgrep {
  escaped_regex="${2//\"/\\\"}"
  RET_MATCH="$(ssh  root@"$SSH_ADDRESS" "grep -$1 \"$escaped_regex\" $3")"
}
...
function find_directory {
  ...
  for metadata_path in $matches_by_name; do
    ...
    metadata="$(ssh  root@"$SSH_ADDRESS" "cat $metadata_path")"
...
function push {
  ...
  while true; do
    if curl --connect-timeout 2 --silent --output /dev/null --form file=@"\"$placeholder\"" http://"$WEBUI_ADDRESS"/upload; then
      ...
      while true; do
        if ssh  root@"$SSH_ADDRESS" stat "/home/root/.local/share/remarkable/xochitl/$RET_UUID.$extension" \> /dev/null 2\>\&1; then
          break
        fi
      done;

      # Delete thumbnails (TODO: Replace thumbnail with pre-rendered thumbnail)
      ssh  root@"$SSH_ADDRESS" "rm -f /home/root/.local/share/remarkable/xochitl/$RET_UUID.thumbnails/*"
      ...
      if [ -z "$directory" ]; then
        ...
        ssh  root@"$SSH_ADDRESS" "sed -i 's/\"parent\": \"[^\"]*\"/\"parent\": \"$2\"/' /home/root/.local/share/remarkable/xochitl/$RET_UUID.metadata"
      ...
      # Directory handling
        ...
        ssh  root@"$SSH_ADDRESS" "sed -i 's/\"type\": \"DocumentType\"/\"type\": \"CollectionType\"/;\
        ...
        ssh  root@"$SSH_ADDRESS" "rm -r /home/root/.local/share/remarkable/xochitl/$RET_UUID{,.cache,.highlights,.pagedata,.pdf,.textconversion,.content}"

        # re-populate *.content
        ssh  root@"$SSH_ADDRESS" "echo "{}" > /home/root/.local/share/remarkable/xochitl/$RET_UUID.content"
...
if [ "$REMOTE" ]; then
  ...
  ssh -o ConnectTimeout=5 -M  -q -f -L "$PORT":"$WEBUI_ADDRESS" root@"$SSH_ADDRESS" -N;
  SSH_RET="$?"

  WEBUI_ADDRESS="localhost:$PORT"
else
  ssh -o ConnectTimeout=1 -M  -f root@"$SSH_ADDRESS" -N
  SSH_RET="$?"
fi
...
if [ "$OUTPUT" ]; then
  ...
  # Multiple directories match
    ...
    match="$(ssh  root@"$SSH_ADDRESS" "$GREP")" # Returns string that includes Metadata Path + Modification date
...
for f in "$@"; do
  ...
  if [ $? == 1 ]; then
    if [ "$OUTPUT" ]; then
      ...
      ssh  root@"$SSH_ADDRESS" "sed -i 's/\"parent\": \"[^\"]*\"/\"parent\": \"$OUTPUT_UUID\"/' /home/root/.local/share/remarkable/xochitl/$ROOT_UUID.metadata"
    fi
...
if [[ "$OUTPUT" || "$DIR_IN_ARG" ]]; then
  if [[ -z "$REMOTE" && -z "$RFKILL" ]]; then
    ssh  root@"$SSH_ADDRESS" "/usr/sbin/rfkill unblock 0"
  fi
